...
g<-function(parms, x){
  ...
  firm_estim<-function(sindex,ap2, ap3, ap4,ap5, abar){
    ...
    findlambda<-function(lambda){
      ...
      E<-rep(1/5,5)
      E<-squarem(E,fixptfn = fxpt, control=list(maxiter=10000000,tol=innertol) )$par
...
ret_objects<-function(parms, x){
  ...
  firm_estim<-function(sindex,ap2, ap3, ap4,ap5, abar){
    ...
    if (round(sindex/outertol)*outertol>0){
      # if end points not of opposite sign, this means either we are hitting a maximum machine readable number issue OR we gamma is 0.
      lambda<-tryCatch({bisection(findlambda, a=0, b=100000, ftol=outertol,xtol=outertol,n=1000000000)$root},
                       error = function(e){
                         if (grepl("end points not of opposite sign",e, fixed=TRUE)){
                           return(Inf)
                         } else{
                           tryCatch({bisection(findlambda, a=0, b=10000, ftol=outertol,xtol=outertol,n=1000000000)$root},
                                    error = function(e){
                                      if (grepl("end points not of opposite sign",e, fixed=TRUE)){
                                        return(Inf)
                                      } else{
                                        tryCatch({bisection(findlambda, a=0, b=1000, ftol=outertol,xtol=outertol,n=1000000000)$root},
                                                 error = function(e){
                                                   if (grepl("end points not of opposite sign",e, fixed=TRUE)){
                                                     return(Inf)
                                                   } else{
                                                     tryCatch({bisection(findlambda, a=0, b=100, ftol=outertol,xtol=outertol,n=1000000000)$root},
                                                              error = function(e){
                                                                if (grepl("end points not of opposite sign",e, fixed=TRUE)){
                                                                  return(Inf)
                                                                } else{
                                                                  tryCatch({bisection(findlambda, a=0, b=10, ftol=outertol,xtol=outertol,n=1000000000)$root},
                                                                           error = function(e){
                                                                             if (grepl("end points not of opposite sign",e, fixed=TRUE)){
                                                                               return(Inf) 
                                                                             } else{
                                                                               tryCatch({bisection(findlambda, a=0, b=1, ftol=outertol,xtol=outertol,n=1000000000, extendInt = 'yes')$root},
                                                                                        error = function(e){
                                                                                          if (grepl("end points not of opposite sign",e, fixed=TRUE)){
                                                                                            return(Inf) 
                                                                                          } else{
