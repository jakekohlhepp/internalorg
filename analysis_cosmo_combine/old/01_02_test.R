...
load("data/01_02_progress_test.RData")

source('small_gmm_objective_lessmod.R')
###############
...
res_store<-gmm(g, x=estim_matrix, t0=coef(res_store), prewhite=0,optfct="optim",centeredVcov='TRUE',weightsMatrix = weight_mat,
               control=list(abstol=1e-16,reltol=1e-16, trace=1, maxit=16000))


save(res_store, innertol, outertol,tau, wage_bound,estim_sample, estim_matrix, file="data/01_02_progress_test.RData")
