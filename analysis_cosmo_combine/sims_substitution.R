...
outertol<-1e-08
############### import results
gamma_vec<-seq(from=0, to=13, length=1000)
alpha<-rep(1/3, 3)
oldwages<-c(6,5,0)+15
...
change_new<-melt(change_out, measure.vars=c("change1", "change2", "change3"), variable="type", value.name="change")
change_new2<-melt(change_out, measure.vars=c("E0_3", "E1_3"), variable="type", value.name="change")

change_new[, `Worker Type`:=gsub("change", "", type)]
ggplot(data=change_new,aes(x=gamma, y=change,color=`Worker Type`, linetype=`Worker Type`))+geom_line(size=1)+
  theme(legend.position="bottom")+ylab("Change in Fraction of Workforce")+xlab("Organization Cost Parameter")

ggsave("out/figures/orgcomplements.png", width=12, heigh=8, units="in")

levels_plot<-melt(change_out, measure.vars=c("E1_1","E1_2", "E1_3"), variable="Worker Type", value.name="share")
levels_plot[, `Worker Type`:=gsub("E1_", "", `Worker Type`)]
ggplot(data=levels_plot,aes(x=gamma, y=share,color=`Worker Type`, linetype=`Worker Type`))+geom_line(size=1)+
  theme(legend.position="bottom")+ylab("Fraction of Workforce")+xlab("Organization Cost Parameter")
ggsave("out/figures/orgcomplements_levels.png", width=12, heigh=8, units="in")



ggplot(data=change_new2,aes(x=gamma, y=change,color=type, linetype=type))+geom_line(size=1)+
  theme(legend.position="bottom")+ylab("Change in Fraction of Workforce")+xlab("Organization Cost Parameter")

change_new2<-melt(change_out, measure.vars=c("E0_1", "E1_1"), variable="type", value.name="change")
ggplot(data=change_new2,aes(x=gamma, y=change,color=type, linetype=type))+geom_line(size=1)+
  theme(legend.position="bottom")+ylab("Change in Fraction of Workforce")+xlab("Organization Cost Parameter")

change_new2<-melt(change_out, measure.vars=c("E0_2", "E1_2"), variable="type", value.name="change")
ggplot(data=change_new2,aes(x=gamma, y=change,color=type, linetype=type))+geom_line(size=1)+
  theme(legend.position="bottom")+ylab("Change in Fraction of Workforce")+xlab("Organization Cost Parameter")
