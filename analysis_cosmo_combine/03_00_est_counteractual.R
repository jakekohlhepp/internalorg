...
quad_wages<-optim(as.numeric(oldwages),quad_solver_no_int,method="L-BFGS-B", lower=rep(0,5), upper=rep(Inf, 5), control=list(trace=1))
counter=1
while (quad_wages$value>0.01 & counter<=10){
  quad_wages<-optim(quad_wages$par,quad_solver_no_int, control=list(trace=1))
  if (quad_wages$value>0.01) quad_wages<-optim(quad_wages$par,quad_solver_no_int,method="L-BFGS-B", lower=rep(0,5), upper=rep(Inf, 5), control=list(trace=1))
  counter<-counter+1
}
...
quad_wages<-optim(as.numeric(oldwages),quad_solver,method="L-BFGS-B", lower=rep(0,5), upper=rep(Inf, 5), control=list(trace=1))
counter=1
while (quad_wages$value>0.01 & counter<=3){
  quad_wages<-optim(quad_wages$par,quad_solver, control=list(trace=1))
  if (quad_wages$value>0.01) quad_wages<-optim(quad_wages$par,quad_solver,method="L-BFGS-B", lower=rep(0,5), upper=rep(Inf, 5), control=list(trace=1))
  counter<-counter+1
}
...
allcombos<-expand.grid(c(TRUE, FALSE),c(TRUE, FALSE),c(TRUE, FALSE) ,c(TRUE, FALSE) ,c(TRUE, FALSE) )
stopifnot(nrow(allcombos)==32)
...
#for (combo in 31){
  ...
  if (length(bindwages)<5){
    quad_solver<-function(w){
      return(sum((solve_min(parms=coef(res_store), gamma_vec=gamma_vec,wages=w, x=estim_matrix)[-bindwages])^2))
    }
    
    quad_wages<-optim(as.numeric(oldwages)[-bindwages],quad_solver,method="L-BFGS-B", lower=rep(0,5-length(bindwages)), upper=rep(Inf,5-length(bindwages)), control=list(trace=1))
    counter=1
    while (quad_wages$value>0.01 & counter<=5){
      quad_wages<-optim(quad_wages$par,quad_solver, control=list(trace=1, maxit=2000))
      if (quad_wages$value>0.01) quad_wages<-optim(quad_wages$par,quad_solver,method="L-BFGS-B", lower=rep(0,5-length(bindwages)), upper=rep(Inf,5-length(bindwages)), control=list(trace=1))
      counter<-counter+1
    }
    tempwage<-rep(20,5)
  ...
  minwage_res<-rbind(minwage_res,data.table(context=paste("Minimum Wage", paste0(bindwages, collapse="-")),mean_wage=as.numeric(weighted.mean(tempwage,minls )),
                                            ...
                                            avg_profit=mean(estim_sample$CSPOP[1]*out$mainstats$new_share/(1-out$mainstats$new_share)/exp(coef(res_store)[1])/tau),
                                            sd_profit=sd(estim_sample$CSPOP[1]*out$mainstats$new_share/(1-out$mainstats$new_share)/exp(coef(res_store)[1])/tau),
                                            consumer_welfare=estim_sample$CSPOP[1]/exp(coef(res_store)[1])*(logSumExp(c(rep(out$mainstats$Q-tau*exp(coef(res_store)[1])*out$mainstats$newprice,estim_sample$weight[1]),0))-digamma(1)),
                                            quality=mean(out$mainstats$Q),
                                            w1=tempwage[1],
                                            w2=tempwage[2],
                                            w3=tempwage[3],
...
## currently not active. not fixed for 5 tasks.
