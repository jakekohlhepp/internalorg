...
for (cnty in c('6037', '36061', '17031') ){
  for (qy in 2021.2 ){
    print(paste("*******no frictions*****",cnty, qy, " -  First Try"))
    ...
    quad_wages<-broyden(solve_reloc, quad_wages$par,maxiter=10000 )
...
for (cnty in c('6037', '17031', '36061') ){
  for (qy in 2021.2 ){
    print(paste("*******no friction*****",cnty, qy, " -  First Try"))
    quad_wages<-BBsolve(as.numeric(initial_wages[county==cnty & quarter_year==qy, c("w1", "w2", "w3", "w4", "w5")]),
...
saveRDS(res_wages, "analysis_final/data/05_05_nofriction.rds")
