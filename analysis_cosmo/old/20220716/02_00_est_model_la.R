...
estim_sample<-firm_quarter[quarter_year %in% c(2019.3,2019.4, 2021.1, 2021.2) & county %in% c(6037)]
# time indicators
...
res_store<-gmm(g, x=estim_matrix, t0=starting, prewhite=0,optfct="optim",centeredVcov='TRUE',weightsMatrix=diag(diag(cov(test))^(-1)),
               control=list(abstol=1e-07,reltol=1e-07, trace=1, maxit=2))

for (counter in 1:50){
  res_store<-gmm(g, x=estim_matrix, t0=coef(res_store), prewhite=0,weightsMatrix=diag(diag(cov(test))^(-1)),optfct="optim",centeredVcov='TRUE',
                 control=list(abstol=1e-07,reltol=1e-07, trace=1, maxit=3000))
  save(res_store,counter, file="data/02_00_progress.RData")
}

maxrefine<-gmm(g, x=estim_matrix, t0=coef(res_store), prewhite=0,weightsMatrix=diag(diag(cov(test))^(-1)),optfct="optim",centeredVcov='TRUE',
