...
starting[15:19]<-0
starting[20:25]<-50
starting<-readRDS('best_starting_05_01.rds')
...
gmm_obj<-function(x){
  gmat<-colMeans(g(parms=x,x=estim_matrix))
  hold<-sum(gmat%*%moment_weights%*%gmat)
  return(hold)
}
...
## particle swarm
#library('pso')
#res_pso<-psoptim(rep(NA,length(starting)),gmm_obj,
#                 lower = c(0,rep(-500, length(starting)-1)), 
#                 upper = rep(1000, length(starting)),
#                 control=list(abstol=1e-8,trace=1,REPORT=1,trace.stats=TRUE))
#
#
### genetic
#library('GA')
...
#         fitness =  function(y) -gmm_obj(y),
#         lower = c(0,rep(-500, length(starting)-1)), upper = rep(1000, length(starting)), 
#         popSize = 50, maxiter = 1000, run = 100, monitor=TRUE)
#
...
for (i in 1:50){
  ...
  res_store<-gmm(g, x=estim_matrix, t0=coef(res_store), prewhite=0,optfct="optim",centeredVcov='TRUE',weightsMatrix=moment_weights,
                 control=list(abstol=1e-07,reltol=1e-07, trace=1, maxit=2000))
  save(res_store, file="data/05_01_progress.RData")
