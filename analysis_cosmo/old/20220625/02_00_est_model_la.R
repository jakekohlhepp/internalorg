...
estim_sample<-firm_quarter[quarter_year %in% c(2019.2,2019.3, 2019.4, 2020.1) & county %in% c(6037)]
# time indicators
...
#starting[c(5,6,7,8,9)]<-starting[c(5,6,7,8,9)]-starting[4]
#starting[c(11,12,13,14,15)]<-starting[c(11,12,13,14,15)]-starting[10]
#starting[c(17,18,19,20,21)]<-starting[c(17,18,19,20,21)]-starting[16]
#starting[c(23,24,25,26,27)]<-starting[c(23,24,25,26,27)]-starting[22]
#starting[4:27]<-0
#starting<-starting[-c(4,10,16,22)]
...
pre_max<-gmm(g, x=estim_matrix, t0=coef(refine_more5), prewhite=0, type="twoStep",optfct="optim",wmatrix='ident',centeredVcov='TRUE',
             control=list(abstol=1e-08,reltol=1e-08, trace=1, maxit=1000))
for (counter in 1:20){
  
  pre_max<-gmm(g, x=estim_matrix, t0=coef(pre_max), prewhite=0, type="twoStep",optfct="optim",wmatrix='ident',centeredVcov='TRUE',
               control=list(abstol=1e-08,reltol=1e-08, trace=1, maxit=1000))
  
}
maxrefine<-gmm(g, x=estim_matrix, t0=coef(pre_max), prewhite=0, type="twoStep",optfct="optim",wmatrix='ident',centeredVcov='TRUE',
                  control=list(abstol=1e-08,reltol=1e-08, trace=1, maxit=10000))


##### gmm evaluation.

save(maxrefine,estim_sample, estim_matrix, starting,innertol, outertol, file="data/02_00_est_res_la.RData")
