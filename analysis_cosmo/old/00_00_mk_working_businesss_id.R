...
setkey(working, bizsub_id, customer_id, date, app_id)
working[, app_ord:=frank(.SD[,c("bizsub_id", "customer_id","date", "app_id")] , ties.method="dense")]
working[, app_ord:=app_ord-app_ord[1], by=c("bizsub_id", "customer_id")]
working[, first_visit:=min(date), by=c("bizsub_id", "customer_id")]
working[, last_visit:=max(date), by=c("bizsub_id", "customer_id")]
working[,cust_count:=uniqueN(customer_id), by=c("bizsub_id", "quarter_year") ]
working[,cust_visits:=uniqueN(.SD[,c("customer_id", "date")]), by=c("bizsub_id", "quarter_year") ]
working[,cust_return:=c(rep(0, .N-1),first(date<last_visit)) , by=c("bizsub_id","customer_id", "date") ]
working[,return_count:=sum(cust_return), by=c("bizsub_id", "quarter_year") ]
working[, female_flag:=max(female_flag,na.rm=TRUE),by=c("bizsub_id", "quarter_year") ]
working[, child_flag:=max(child_flag,na.rm=TRUE),by=c("bizsub_id", "quarter_year") ]
working[, male_flag:=max(male_flag,na.rm=TRUE),by=c("bizsub_id", "quarter_year") ]
...
staff_task<-working[, .(count=.N, duration=as.double(sum(duration)),
                      revenue=sum(price)),
                    by=c("staff_id", "bizsub_id","COUSUB","CSPOP", "county",
                         "rep_text_cluster", "quarter_year", "business_id", "clust", "location_state","cust_count", "return_count", "cust_visits","female_flag", "male_flag","child_flag")]
staff_task[,c("tot_duration", "emps", "service_types", "revenue") := list(sum(duration), uniqueN(staff_id), uniqueN(clust), sum(revenue)), by=c("bizsub_id", "quarter_year")]
staff_task[,c("emp_tot") := list(sum(duration)), by=c("bizsub_id", "quarter_year", "staff_id")]
staff_task[,staff_num:= frank(staff_id, ties.method="dense"), by=c("bizsub_id", "quarter_year")]
tm<-staff_task[, .(task_mix=sum(duration/tot_duration), rev_per_time=sum(revenue)/sum(tot_duration)), by=c("bizsub_id", "clust", "quarter_year")]
tm<-dcast(tm,bizsub_id+quarter_year~clust, value.var=c("task_mix", "rev_per_time" ))
staff_task[, jobvect:=duration/emp_tot]

# cast first by cluster
staff_task<-dcast(staff_task, location_state+COUSUB+CSPOP+county+bizsub_id + quarter_year+business_id+cust_count+return_count+cust_visits+tot_duration+staff_num+emps+emp_tot+service_types+revenue+female_flag+male_flag+child_flag~clust, value.var=c("rep_text_cluster", "jobvect"))
staff_task[,efrac:=emp_tot/tot_duration]

staff_task<-merge(staff_task,tm, by=c("bizsub_id", "quarter_year") )
...
for (col in names(staff_task)[grep("^jobvect", names(staff_task))]) staff_task[,(paste0("max_",col))  := max(get(col),na.rm=TRUE) , by=c("bizsub_id","quarter_year")]
for (col in names(staff_task)[grep("^jobvect", names(staff_task))]) staff_task[,(paste0("min_",col))  := min(get(col),na.rm=TRUE) , by=c("bizsub_id","quarter_year")]
...
firm_quarter<-staff_task[tot_duration>0, .(s_index=sum(mi_part)), by=c("location_state","COUSUB","bizsub_id","business_id", "quarter_year" ,"COUSUB","CSPOP", "county",
...
firm_quarter[ , d_2:=distfun(task_mix2), by=c("quarter_year","COUSUB")]
firm_quarter[ , d_3:=distfun(task_mix3), by=c("quarter_year","COUSUB")]
firm_quarter[ , d_4:=distfun(task_mix4), by=c("quarter_year","COUSUB")]
firm_quarter[ , d_5:=distfun(task_mix5), by=c("quarter_year","COUSUB")]
firm_quarter[ , d_6:=distfun(task_mix6), by=c("quarter_year","COUSUB")]

firm_quarter[ , s_2:=sum(task_mix2)-task_mix2, by=c("quarter_year", "COUSUB")]
firm_quarter[ , s_3:=sum(task_mix3)-task_mix3, by=c("quarter_year", "COUSUB")]
firm_quarter[ , s_4:=sum(task_mix4)-task_mix4, by=c("quarter_year", "COUSUB")]
firm_quarter[ , s_5:=sum(task_mix5)-task_mix5, by=c("quarter_year", "COUSUB")]
firm_quarter[ , s_6:=sum(task_mix6)-task_mix6, by=c("quarter_year", "COUSUB")]


### more product space instruments
firm_quarter[ , m_serv:=sum(male_flag)-male_flag, by=c("quarter_year", "COUSUB")]
firm_quarter[ , child_serv:=sum(child_flag)-child_flag, by=c("quarter_year", "COUSUB")]
...
firm_quarter<-merge(firm_quarter, qcew[!is.na(county),c("county", "quarter_year", "avg_wage_qtr")], by=c("county", "quarter_year"),
                    all.x=TRUE)
