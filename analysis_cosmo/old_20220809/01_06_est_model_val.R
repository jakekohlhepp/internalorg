...
estim_sample<-firm_quarter[quarter_year %in% c(2021.2) & county %in% c(36061, 6037)]
## county indicator
...
g<-function(parms, x){
  ...
  firm_estim<-function(sindex,ap2, ap3, ap4,ap5, ap6, abar, isla){
    ...
    findlambda<-function(lambda){
      ...
      obj_fn<-function(p){
        a<-t(t(A*p)/colSums(A*p)*aleph)
        return(sum(a*spec_log(t(t(a/rowSums(a))/colSums(a)))) +sum(a*lambda*(wmat -h/rho[1+isla]/abar) ))
      }
      E<-rep(1/6,6)
      E<-squarem(E,fixptfn = fxpt,objfn=obj_fn, control=list(maxiter=100000,tol=innertol) )$par
      #old<-Inf
      #icount<-0
      #while (any(abs(obj_fn(E)-old)>innertol) & icount<1000000){
      #  old<-obj_fn(E)
      #  E<-fxpt(E)
      #  print(obj_fn(E))
      #  icount<-icount+1
  ...
  Z<-cbind(1,x[,2:6],x[,1],x[,2:6]*x[,1])
  Z<-cbind(moment_mat[,1]*Z,moment_mat[,2]*Z)
  wagemom<-x[,11]-W*x[,10]*x[,12]
  return(penalty*cbind(Z,wagemom, Z*x[,13], wagemom*x[,13])  )
}
...
save(res_store, innertol, outertol, wage_bound,estim_sample, estim_matrix, file="data/01_06_progress.RData")
...
save(res_store, innertol, outertol, wage_bound,estim_sample, estim_matrix, file="data/01_06_final_joint.RData")
