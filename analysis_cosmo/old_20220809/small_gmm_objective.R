...
g<-function(parms, x){
  ...
  res<-mclapply(1:nrow(x), function(y){firm_estim(x[y,1], x[y,2], x[y,3], x[y,4],x[y,5],x[y,6],x[y,11]
  )}, mc.cores=core_count)
  ...
  moment_mat<-cbind(x[,7]-1/rho/(1-x[,8]) - x[,11]*(W + gamma*x[,1])-x[,2:6]%*%cmat1-c_bar,
                    log(x[,8]/x[,9]) +rho*x[,7]-xi-u_bar-x[,2:6]%*%umat)
  
  Z<-cbind(1,x[,2:6],x[,1],x[,2:6]*x[,1], x[,10:10])
  wagemom<-x[,12]-W*x[,11]*x[,13]
...
ret_objects<-function(parms, x){
  ...
  cost_qual<-x[,11]*(W + gamma*x[,1])-xi/rho
  wage_qual<-x[,11]*W -xi/rho
  moment_mat<-cbind(x[,7]-1/rho/(1-x[,8]) - x[,11]*(W + gamma*x[,1])-x[,2:6]%*%cmat1-c_bar,
                    log(x[,8]/x[,9]) +rho*x[,7]-xi-u_bar-x[,2:6]%*%umat)
  
  Q<-xi+u_bar+x[,2:6]%*%umat+moment_mat[,2]
  C<-x[,11]*(W + gamma*x[,1]) + x[,2:6]%*%cmat1 +c_bar+moment_mat[,1]
