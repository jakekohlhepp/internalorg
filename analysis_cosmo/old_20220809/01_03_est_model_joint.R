...
g<-function(parms, x){
  ...
  firm_estim<-function(sindex,ap2, ap3, ap4,ap5, ap6, abar, isla){
    ...
    findlambda<-function(lambda){
      ...
      old<-rep(Inf,6)
      icount<-0
      while (any(abs(E-old)>innertol) & icount<1000000){
        old<-E
        E<-fxpt(E)
        icount<-icount+1
      }
  ...
  # helper functions
  finda<-Vectorize(function(y){res[y][[1]][1]})
  ret_gamma<-Vectorize(function(y){(res[y][[1]][[3]])^(-1) })
...
save(res_store, innertol, outertol, wage_bound,estim_sample, estim_matrix, file="data/01_03_progress.RData")
...
save(res_store, innertol, outertol,tau, wage_bound,estim_sample, estim_matrix, file="data/01_03_final_joint.RData")
