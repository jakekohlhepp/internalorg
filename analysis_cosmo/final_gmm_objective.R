...
g<-function(parms, x){
  ...
  res<-mclapply(1:nrow(x), function(y){firm_estim(x[y,1], x[y,2], x[y,3], x[y,4],x[y,5],x[y,6],x[y,9]
  )}, mc.cores=core_count)
  ...
  moment_mat<-cbind(x[,7]-1/rho/(1-x[,8]) - x[,9]*(W + gamma*x[,1])-x[,2:6]%*%cmat1-c_bar,
                    log(x[,8]/outside_share) +rho*x[,7]-xi-u_bar-x[,2:6]%*%umat)
  
  Z<-cbind(1,x[,2:6],x[,1],x[,2:6]*x[,1], x[,9])
  wagemom<-wage_match-W*x[,9]*pop_subdiv*x[,8]
  return(penalty*cbind(moment_mat[,1]*Z,moment_mat[,2]*Z,wagemom)  )
  #return(penalty*cbind(moment_mat[,1]*Z,moment_mat[,2]*Z)  )
}

gmm_obj<-function(x){
  hold<-g(parms=x,x=estim_matrix)
...
ret_objects<-function(parms, x){
  ...
  C<-x[,9]*(W + gamma*x[,1]) + x[,2:6]%*%cmat1 +c_bar+moment_mat[,1]
