...
res_store<-gmm(g, x=estim_matrix, t0=starting, prewhite=0,optfct="optim",centeredVcov='TRUE',weightsMatrix=moment_weights,
               control=list(abstol=1e-07,reltol=1e-07, trace=1, maxit=2000))
save(res_store, file="data/01_01_progress.RData")
...
for (i in 1:20){
  ...
  res_store<-gmm(g, x=estim_matrix, t0=coef(res_store), prewhite=0,optfct="optim",centeredVcov='TRUE',weightsMatrix=moment_weights,
                 control=list(abstol=1e-07,reltol=1e-07, trace=1, maxit=2000))
  save(res_store, innertol, outertol,tau, wage_bound,estim_sample, estim_matrix,moment_weights, file="data/01_01_progress.RData")
...
save(res_store, innertol, outertol,tau, wage_bound,estim_sample, estim_matrix,moment_weights, "data/01_01_final_ny.RData")
