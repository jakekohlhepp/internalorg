...
countypop<-fread('../mkdata/raw/20220711_2020xwalk/geocorr2022_2219203286.csv')
countypop[,CSPOP:=pop20]
countypop[,COUSUB:=cousub20 ]
countypop<-unique(countypop[,c("county", "COUSUB", "CSPOP")])

# attach county subdivisions.
data<-fread('../mkdata/raw/20220711_2020xwalk/geocorr2022_2219204622.csv')
...
# only map when 50 percent of zcta is within one subdiv.
data<-data[afact>0.95 | count==1]
# should have uniqueness.
stopifnot(uniqueN(data[zcta %in% unique(firm_quarter$location_zip) ]$zcta)==nrow(data[zcta %in% unique(firm_quarter$location_zip) ]))
data[, location_zip:=zcta]
# using 
firm_quarter<-merge(firm_quarter,data[,c("CSPOP", "COUSUB", "location_zip", "county")], by="location_zip", all.x=TRUE)
firm_quarter[, salon_share_subdiv:=cust_count/CSPOP]
