...
#### Step -1: Remove task 5, which is the stopword task.
working<-working[clust!=5,]
...
staff_task<-working[, .(count=.N, duration=as.double(sum(duration)),
                    ...
                    by=c("staff_id", "location_id","location_zip",
                         "rep_text_cluster", "quarter_year", "business_id", "clust", "location_city", "location_state","cust_count", "return_count", "cust_visits",
                         "male_flag", "female_flag", "child_flag")]
...
staff_task<-dcast(staff_task, location_state+location_city+location_zip+location_id + quarter_year+business_id+cust_count+return_count+cust_visits+tot_duration+staff_num+emps+emp_tot+service_types+revenue+female_flag+child_flag+male_flag~clust, value.var=c("rep_text_cluster", "jobvect"))
staff_task[,efrac:=emp_tot/tot_duration]

staff_task<-merge(staff_task,tm, by=c("location_id", "quarter_year") )
names(staff_task)[str_detect(names(staff_task), "task_mix")]<-to("task_mix",4)
# zero out all.
...
firm_quarter<-staff_task[tot_duration>0, .(s_index=sum(mi_part)), by=c("location_state","location_city", "location_zip","location_id","business_id", "quarter_year", 
                                                   "emps", "service_types","tot_duration","revenue","cust_count","return_count","cust_visits",
...
firm_quarter[,s_max:=-task_mix1*spec_log(task_mix1)-task_mix2*spec_log(task_mix2)-task_mix3*spec_log(task_mix3)-task_mix4*spec_log(task_mix4)]
...
aug_firm_quarter[ , d_2:=distfun(task_mix2), by=c("quarter_year","county")]
aug_firm_quarter[ , d_3:=distfun(task_mix3), by=c("quarter_year","county")]
aug_firm_quarter[ , d_4:=distfun(task_mix4), by=c("quarter_year","county")]
aug_firm_quarter[ , d_5:=distfun(task_mix5), by=c("quarter_year","county")]
aug_firm_quarter[ , d_6:=distfun(task_mix6), by=c("quarter_year","county")]
aug_firm_quarter[,d_sum:=sqrt(d_2+d_3+d_4+d_5+d_6)]


aug_firm_quarter[ , s_2:=sum(task_mix2)-task_mix2, by=c("quarter_year", "county")]
aug_firm_quarter[ , s_3:=sum(task_mix3)-task_mix3, by=c("quarter_year", "county")]
aug_firm_quarter[ , s_4:=sum(task_mix4)-task_mix4, by=c("quarter_year", "county")]
aug_firm_quarter[ , s_5:=sum(task_mix4)-task_mix4, by=c("quarter_year", "county")]
aug_firm_quarter[ , s_6:=sum(task_mix6)-task_mix1, by=c("quarter_year", "county")]
aug_firm_quarter[,s_sum:=s_2+s_3+s_4+s_5+s_6]
...
aug_firm_quarter[,county_helper:=NULL]
aug_firm_quarter[!is.na(rev_per_time_4), county_helper:=sum(rev_per_time_2, na.rm=TRUE), by=c("quarter_year", "county")]
aug_firm_quarter[, county_helper:=ifelse(is.na(county_helper), 0, county_helper)]
