...
working_data<-data.table(readRDS("C:/Users/jakek/jmp_dont_backup/data/compiled_trxns.rds"))
working_data[,raword:=1:.N]
# limit to just self-reported hair salons prior to covid-19
lim_working<-working_data[industry%in% c("Hair Salon", "hairSalon","Barber Shop",
                                         "Blowouts & Styling") ,]
rm(working_data)
gc()
...
clean<-function(x){
  return(str_replace_all(x, regex("\\W+"), " "))
}
clean<-Vectorize(clean)
text_to_clean[,clean_task:=str_trim(clean(clean_task))]
# because it is so important we separate hair cut into two words.
text_to_clean[, clean_task:=str_replace(clean_task,"haircut", "hair cut")]
...
final_groups<-unique(data.table(clust=as.numeric(cutree(clusters, clust_num)), clean_task=names(cutree(clusters, clust_num))))
final_groups<-merge(final_groups,text_to_clean, by="clean_task", all.y=TRUE)
final_groups[is.na(clust), clust:=clust_num+1]
