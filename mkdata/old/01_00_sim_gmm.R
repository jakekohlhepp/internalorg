...
I<-sapply(1:firm_count,function(x){ runif(1,min=0, max=entropy(alpha[x,]))})
...
for (t in 1){
  ...
  w_base<-c(0.11, 0.11, 0.11, 0.11)
  w_slope<-c(1.89, 1.93, 1.85, 1.9)
                     ...
                     error = function(e){
                       if (findlambda(100)>0){
                         return(100)
                           ...
                           return(0)}}) # when there is no root in range, it means constraint does not bind.
