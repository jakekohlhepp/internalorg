...
## choose one reference firm-quarter from each county with N types. use the largest firm in terms of emp count
staff_comparable_fivers<-staff_merged_within[types_observed_firm==CONFIG$n_worker_types & service_mix_id== all_tasks ]
...
for (cur_loc_quarter in unique(staff_comparable$year_loc)){
  ...
  allcombos<-combn(1:CONFIG$n_worker_types, nrow(mat), simplify=FALSE)
...
colnames(label_final)<-c("year_loc", paste0("type_within_firm",1:CONFIG$n_worker_types))
...
# first every type
expanded_data<-data.table()
for (i in 1:CONFIG$n_worker_types ) expanded_data<-rbind(expanded_data,unique(staff_merged_within[, .SD, .SDcols=c(cols_tokeep) ]))
expanded_data[, worker_type:=1:.N, by=cols_tokeep]

temp<-copy(expanded_data)
for (i in 1:(CONFIG$n_worker_types - 2) ) temp<-rbind(temp,expanded_data)
expanded_data<-copy(temp)
rm(temp)
expanded_data[, compare_type:=setdiff(1:CONFIG$n_worker_types,worker_type), by=c(cols_tokeep, "worker_type")]
...
for (cnty in CONFIG$counties_padded){
  ...
  if (!exists("bootreps")){
  png(paste0("results/out/01_02_network_2019_",cnty,".png"), pointsize=10, width=1400, height=1200, res=300)
...
for (cnty in CONFIG$counties_padded){
  ...
  qgraph_mat[qgraph_mat<2]<-0
  qgraph_mat[qgraph_mat>=2]<-1
  #png(paste0("results/out/01_02_network_",cnty,".png"), pointsize=10, width=1400, height=960, res=300)
