...
#' location-level weights and re-runs the two-stage wage / price solve from
#' 06_estimation.R. Used to produce standard errors downstream in
...
min_wage <- data.table(estimation_sample$min_wage_levels)

source('preamble.R')
estimation_objects <- build_estimation_setup(working_data, estim_matrix, config = CONFIG)
beta <- estimation_objects$beta

## draw weights for each location (Bayesian bootstrap with exponential weights).
set.seed(833927)
bootreps <- 200
...
for (iter in 1:bootreps) {
  ...
  boot_weight <- rbind(boot_weight, data.table(iteration = iter,
                                               location_id = unique(working_data$location_id),
                                               bweight = temp_weight))
}
saveRDS(boot_weight, file.path("results", "data", "07_boot_weights.rds"))
boot_res <- data.table()
...
for (iter in 1:bootreps) {
  ...
  ## do first regression (weighted IV on the refactored Hausman instrument).
  xnam <- c("factor(county):factor(quarter_year)",
            paste0("factor(county):avg_labor:",
                   names(working_data)[grep("^B_raw_[0-9]_", names(working_data))]))
  xnam <- paste0("log_rel_mkt~", paste0(xnam, collapse = "+"),
                 "-1|factor(county):cust_price |factor(county):hausman_other_price")
  m_iv <- ivreg(eval(xnam), data = working_data, weights = boot_weight)
  ...
  objective_squared <- function(parms) {
    res <- colSums(sweep(objective_gmm(theta = parms, x = estim_matrix,
                                        beta = beta, beta_2_subset = beta_2_subset,
                                        config = CONFIG),
                         MARGIN = 1,
                         working_data$boot_weight / sum(working_data$boot_weight), `*`))
    return(sum(res^2))
  }

  objective_vect <- function(parms) {
    return(colSums(sweep(objective_gmm(theta = parms, x = estim_matrix,
                                        beta = beta, beta_2_subset = beta_2_subset,
                                        config = CONFIG),
                         MARGIN = 1,
                         working_data$boot_weight / sum(working_data$boot_weight), `*`)))
  ...
  working_data[, gamma_invert := get_gammas(coef_vect, estim_matrix,
                                             beta = beta, beta_2_subset = beta_2_subset,
                                             config = CONFIG)]
  working_data[, p_adj := cust_price - wb_2 - wb_3 - wb_4 - wb_5 -
                 gamma_invert * s_index +
                 mk_piece / beta[paste0("factor(county)", county, ":cust_price"), ]]

  first_try <- coef(lm(p_adj ~ avg_labor:factor(county):factor(quarter_year) +
                         factor(county):factor(quarter_year) +
                         factor(quarter_year):(task_mix_2 + task_mix_3 + task_mix_4 + task_mix_5) - 1,
                       data = working_data, weights = boot_weight))

  # need constraints

  temp <- data.table(V1 = names(first_try), position = 1:length(first_try))
  temp[, quarter_year := as.numeric(str_extract(V1, "\\b\\d{4}\\.\\d\\b"))][
    , county := as.numeric(gsub("factor\\(county\\)", "",
                                 str_extract(V1, "factor\\(county\\)\\s*(\\d+)")))]
  ...
  temp2[, county := as.numeric(gsub("factor\\(county\\)", "",
                                     str_extract(V1, "factor\\(county\\)\\s*(\\d+)")))]


  temp <- merge(temp2, temp, by = "county", allow.cartesian = TRUE)
  min_wage_j <- copy(min_wage)
  min_wage_j[, county := as.numeric(county)]
  temp <- merge(temp, min_wage_j, by = c("county", "quarter_year"), all.x = TRUE)
  ...
  obj_final_reg <- function(x) {
    res <- colSums(sweep(matrix(working_data$p_adj - mod_mm_2 %*% x,
                                 nrow = nrow(mod_mm_2), ncol = ncol(mod_mm_2), byrow = FALSE) * mod_mm_2,
                         MARGIN = 1,
                         working_data$boot_weight / sum(working_data$boot_weight), `*`))
    return(sum((res)^2))
  }
  ...
  final_reg <- optim(starting_final_reg, obj_final_reg,
                     lower = lower_bound, upper = rep(Inf, length(starting_final_reg)),
                     method = "L-BFGS-B", control = list(maxit = 1000000, trace = 3, xtol = obj_tol))
  print(final_reg)
  ...
  saveRDS(boot_res, file.path("results", "data", "07_bootstrap.rds"))
