## remove all heterogeneity within market in gamma.
...
counterfactual_context <- load_counterfactual_context(extra_packages = c("ggplot2"))
...
## set gamma to be the average gamma in each market (county-quarter)
working_data[, gamma_invert:=median(gamma_invert, na.rm=TRUE), by=c("county", "quarter_year")]
...
for (cnty in c('36061','17031') ){
  for (qy in 2021.2 ){
    print(paste("******no variance in gamma***",cnty, qy, " -  First Try"))
    quad_wages<-BBsolve(as.numeric(initial_wages[county==cnty & quarter_year==qy, c("w1", "w2", "w3", "w4", "w5")]),
...
### use initial wages rather than reallocation wages as starting point for los angeles.
for (cnty in c('6037') ){
  for (qy in 2021.2 ){
    print(paste("*******no variance in gamma***",cnty, qy, " -  First Try"))
    quad_wages<-BBsolve(as.numeric(initial_wages[county==cnty & quarter_year==qy, c("w1", "w2", "w3", "w4", "w5")]),
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=500000, M=c(10,2,200,100,15,50,500)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=300000, M=c(10,2,200,100,15)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=200000, M=c(10,2,200,100,15)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=100000, M=c(100,2,200,10,15)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=90000, M=c(10,2,15,5,20)))
    
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=80000, M=c(10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=70000, M=c(10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=60000, M=c(10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=50000, M=c(10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=40000, M=c(10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=30000, M=c(10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=10000, M=c(100,10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=10000, M=c(100,10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=5000, M=c(100,10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=4000, M=c(100,10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=3000, M=c(100,10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=2000, M=c(100,10,2,15,5,20)))
    quad_wages<-BBsolve(quad_wages$par,
                        solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=1000, M=c(100,10,2,15,5,20)))
    #quad_wages<-BBsolve(quad_wages$par,
    #                    solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=100, M=c(100,10,2,15,5,20)))
    #quad_wages<-BBsolve(quad_wages$par,
    #                    solve_wages, control=list(maxit=10000,trace=TRUE, NM=TRUE, noimp=200, tol=1, M=c(100,10,2,15,5,20)))
...
save_counterfactual_rds(
  res_wages,
  "05_01_remove_variance.rds",
  legacy_filename = "05_01_remove_variance.rds"
)
...
save_counterfactual_rds(
  prod_data_new,
  "05_01_prod_remove_variance.rds",
  legacy_filename = "05_01_prod_remove_variance.rds"
)


### the goal of this counterfactual is to analyze the dispersion in productivity.
...
stopifnot(all(is.na(prod_data_initial$sol_type)))
prod_data_initial[, agg_prod:=Q+C*rho[county]]
prod_data_initial[, rev_prod:=newprice/avg_labor]
firm_initial_model<-melt(prod_data_initial,id.vars=c("rev_prod","agg_prod","location_id", "county" ,"s_index"), measure.vars=patterns("^B_[0-9]") )
firm_initial_model<-firm_initial_model[,.(tot_prod=sum(value)), c("rev_prod","agg_prod","location_id", "county" ,"s_index")]
firm_initial_model[, sol_type:="Initial"]
## new model based
prod_data_new[, agg_prod:=Q+C*rho[county]]
prod_data_new[, rev_prod:=newprice/avg_labor]
firm_new_model<-melt(prod_data_new,id.vars=c("rev_prod","agg_prod","location_id", "county" ,"s_index","sol_type"), measure.vars=patterns("^B_[0-9]") )
firm_new_model<-firm_new_model[,.(tot_prod=sum(value)), c("rev_prod","agg_prod","location_id", "county" ,"s_index","sol_type")]

firm_all<-rbind(firm_new_model[sol_type=="reorg", ],firm_initial_model)

ggplot(data=firm_all[county=='36061'],aes(x=s_index, fill=sol_type))+
  geom_histogram(position = "identity", alpha=0.8,color="black")+
  theme_bw()

ggplot(data=firm_all[county=='6037'],aes(x=rev_prod, fill=sol_type))+
  geom_histogram(position = "identity", alpha=0.7,color="black")+
  theme_bw()

firm_initial_model<-firm_initial_model[, .(s_avg=mean(s_index),
                            s_sd=sd(s_index),
                            s_p25=quantile(s_index,0.25),
                            s_p75=quantile(s_index,0.75),
                            prod_avg=mean(rev_prod),
                            prod_sd=sd(rev_prod),
                            p25=quantile(rev_prod,0.25),
                            p75=quantile(rev_prod,0.75)), by=c("county")]

firm_new_model<-firm_new_model[, .(s_avg=mean(s_index),
                                          s_sd=sd(s_index),
                                          s_p25=quantile(s_index,0.25),
                                          s_p75=quantile(s_index,0.75),
                                          prod_avg=mean(rev_prod),
                                          prod_sd=sd(rev_prod),
                                          p25=quantile(rev_prod,0.25),
                                          p75=quantile(rev_prod,0.75)), by=c("county", "sol_type")]
