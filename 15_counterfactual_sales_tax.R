## Sales-tax counterfactual.
## Scales rho per county to mimic sales-tax pass-through, then re-solves
## wages and writes both the new productivity panel and the original-rho
## productivity panel for comparison.
source("config.R")
...
for (cnty in CONFIG$counties) {
  for (qy in get_counterfactual_focus_quarter()) {
    print(paste("*******", cnty, qy, "- shared", "sales_tax", "reorg solve"))
    ...
    baseline_start <- as.numeric(unlist(
      initial_wages[county == cnty & quarter_year == qy,
                    paste0("w", seq_len(n_worker_types)), with = FALSE],
      use.names = FALSE))
    target_labor <- as.numeric(unlist(
      total_labor[county == cnty & quarter_year == qy, -c("county", "quarter_year")],
      use.names = FALSE
    ))
    protected_starts <- counterfactual_protected_worker_starts(
      realloc_start,
      baseline_start,
...
### save productivity panel under shocked rho.
