...
*Last updated: 2026-03-12 (second-pass review)*
...
`C:/Users/jakek/jmp_dont_backup/`. These are:
...
`jmp_dont_backup` subdirectories (`20200909_raw/Marketing Intern Data/` and
...
4. Replace hard-coded `jmp_dont_backup` paths with `file.path(CONFIG$raw_data_base, CONFIG$<subpath>)`.
...
**Note:** The Step 6 example in the original draft plan used a `run_if_needed()`
