...
| `--array=1-200%50` | 200 reps, 50 concurrent | `CONFIG$bootstrap_reps` defaults to 200 in [config.R](../config.R). Reps are independent, each uses 1 CPU, so %50 finishes the array in ~4 waves on the general partition. |
