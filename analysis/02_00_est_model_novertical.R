...
g<-function(parms, x){
  ...
  moment_mat<-cbind(x[,5]-1/rho/(1-x[,6]) - W - gamma*x[,1]-c_bar-ct1*x[,12]-ct2*x[,13]-ct3*x[,14],
                    log(x[,6]/x[,7]) +rho*x[,5]-xi-u_bar
                    -ut1*x[,12]-ut2*x[,13]-ut3*x[,14])
...
#
#res<-maxNR(gmm_obj, start=starting, print.level=2)
############### 
...
outgmm4<-gmm(g, x=estim_matrix, t0=coef(outgmm3), prewhite=0, type="twoStep",optfct="optim",wmatrix='ident',centeredVcov='TRUE',
             control=list(abstol=1e-08,reltol=1e-08, trace=1, maxit=10000))
outgmm5<-gmm(g, x=estim_matrix, t0=coef(outgmm5), prewhite=0, type="twoStep",optfct="optim",wmatrix='ident',centeredVcov='TRUE',
...
save(outgmm3,estim_sample, estim_matrix, starting,innertol, outertol,N,T, file="data/02_00_est_res_ny.RData")
