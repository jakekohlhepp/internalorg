...
if(helper){
c<-400
alpha<-c(0.1,0.3, 0.2, 0.25,0.15,0)
xi<- 0.5
xi_bar<-1
...
solve_model<-function(c, alpha, xi, xi_bar, w, delta, theta){
  ...
  doalgo<-function(l2){
    ...
    res<-uniroot(ba,l2=l2, lower=0, upper=100, check.conv = TRUE)
    return(c-calc_c(ba_all(res$root,l2), l2))
  }
  doalgo<-Vectorize(doalgo)
  #final_res<-sapply(seq(from=0.3, to=40, by=1),doalgo, c=5,xi=2.3,xi_bar=3,alpha=alpha)
  final_res<-uniroot.all(doalgo,lower=0, upper=1)
  l2<-final_res$root
  l1<-uniroot(ba,l2=l2, lower=0, upper=100, check.conv = TRUE)
  ...
  return(list(gamma=1/final_res$root,
...
if (helper) solve_model(c=c, alpha=alpha, xi=xi, xi_bar=xi_bar,w=w, delta=delta, theta=theta )
