### remove variables from estimation to find issue
### should get well-defined covariance matrix.
...
# true
starting<-c(1.01,0.5,0.024, 2,0.01, 0.01, 0.01, 0.01,1.96, 1.96, 1.96, 1.96)
g<-function(parms, x){
  ...
  w<-c(parms[5],parms[6],parms[7], 0,
       parms[9], parms[10], parms[11], parms[12])
  #w<-c(0.01, 0.01, 0.01, 0.01,
  #     1.96, 1.96, 1.96, 1.96)
  #beta<-c(parms[5],parms[6],parms[7],parms[8])
  beta<-c(1,1,1,1)
  #omega_bar<-parms[5]
  omega_bar<-exp(0+0.4^2/2)
  #nu_bar<-parms[5]
  nu_bar<-exp(0+0.4^2/2)
...
res<-gmm(g1, x1, c(0,0), prewhite=0, type="twoStep",optfct="optim",wmatrix='ident',centeredVcov='TRUE')
