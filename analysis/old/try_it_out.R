...
for (t in 1){
  firm_essentials<-data.table(market=seq(from=1, to=firm_count, length=1),
                              ...
                              rent_shock=exp(rnorm(firm_count, mean=-1)),
                              q_shock=rnorm(firm_count, mean=0, sd=0.2))
  firm_essentials[, c_shock:=rent_shock+exp(rnorm(firm_count, mean=-11))]
  # this iter vars
  delta<-delta_0
  rho<-rho_0
  beta<-beta_0
  ...
  w_base<-c(1, 1, 1, 1)
  w_slope<-c(2.3, 1.9, 1.9, 1.9)
  w<-w_base+rep(w_slope,2)
  w[1:T]<-w[1:T]-w_slope
  ...
  firm_estim<-function(sindex, ap1, ap2, ap3, ap4){
    ...
    findlambda<-function(lambda){
      ...
      E0<-rep(1/(N*T),N*T)
      pf<-squarem(p=E0, y=1, fixptfn=fxpt, objfn=objval, control=list(tol=innertol, method=3, step.min0=1))
    ...
    lambda<-tryCatch({uniroot(findlambda, lower=0, upper=100, tol=outertol)$root},
                     error = function(e){
                       if (findlambda(100)>0 & findlambda(0)>0 ){
                         return(100)
                       } else if (findlambda(0)<=0 & findlambda(100)<=0) {
                         return(0)
                       } else if (findlambda(100)) {
                         return(NA)
                       }
  ...
  firm_essentials[, top:=exp(q_shock+task_mix1*beta[1] +task_mix2*beta[2]+task_mix3*beta[3]
                             +findd(firm_id)-
                               rho*c_shock -
                               rho*(task_mix1*c[1] +task_mix2*c[2]+task_mix3*c[3] ))]
  ...
  firm_essentials[, mc:=wagebill+task_mix1*c[1] +task_mix2*c[2]+task_mix3*c[3] +c_shock]
  firm_essentials[,price:=mc+1/rho_0]
  firm_essentials[,s_0:=1-sum(share),by="market"]
  firm_essentials[,det_util:=qual-rho_0*price+task_mix1*beta[1] +task_mix2*beta[2]+task_mix3*beta[3]+task_mix4*beta[4]]
  ...
  bmaxes<-data.table(t(sapply(firms_see, function(x){
    ...
    hold[zero,]<-0
    return(colMax(hold/rowSums(hold)))
  })) )  
  ...
  bmines<-data.table(t(sapply(firms_see, function(x){
    ...
    zero<-(round(rowSums(finda(x)[[1]]),3)==0)
    hold[zero,]<-0
    return(colMin(hold/rowSums(hold)))
  })) )
