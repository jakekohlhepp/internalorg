## estimate model.
## now do alternative demand system.
...
estim_sample<-firm_quarter[quarter_year %in% c(2019.4) & county %in% c(6037)]
...
estim_matrix<-as.matrix(estim_sample[,c("s_index", "task_mix2", "task_mix3", "task_mix4",
                                        "cust_price", "salon_share", "s_2", "s_3","s_4")])
...
g<-function(parms, x){
  ...
  income<-parms[6]
  mu<-parms[7]
  w<-c(0,parms[8],parms[9],parms[10],
  ...
  firm_estim<-function(sindex,ap2, ap3, ap4){
    ...
    findlambda<-function(lambda){
      A<-exp(-lambda*(wmat -thetamat*income*delta^h) )
      A[A>=Inf]<-1e8
      fxpt<-function(p){
  ...
  moment_mat<-cbind(x[,5]-(xi+u_bar)*income - mu^(1/rho)*x[,6],
                    x[,5]-((xi+u_bar)*income+W*rho )/(1+rho) )
  Z<-cbind(1,x[,2:4],x[,1],x[,2:4]*x[,1])
